...
export function CourseProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            const saved = localStorage.getItem("entra-completedLessons");
            if (saved) {
                setCompletedLessons(new Set(JSON.parse(saved)));
            }
            const savedTrainerMode = localStorage.getItem("entra-trainerMode");
            if (savedTrainerMode) {
    ...
    useEffect(() => {
        if (typeof window !== "undefined" && completedLessons.size > 0) {
            localStorage.setItem("entra-completedLessons", JSON.stringify([...completedLessons]));
        }
    }, [completedLessons]);
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            localStorage.setItem("entra-trainerMode", JSON.stringify(trainerMode));
        }
    }, [trainerMode]);
